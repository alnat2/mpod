...
| Размер очереди ($N$) | Создано объектов `new Audio()` | Всего Range-запросов | Суммарный трафик проб | Время до завершения всех проб | Состояние пула HTTP/1.1 и поведение браузера |
|---|---|---|---|---|---|
| **1 выпуск** | 1 | 2 | 65.5 КБ | 48 мс | 1 соединение используется, пул свободен |
| **10 выпусков** | 10 | 22 | 718.4 КБ | 382 мс | Исчерпание пула из 6 соединений: запросы 7–10 встают в очередь (stalled ~140 мс) |
| **30 выпусков** | 30 | 66 | 2 184.2 КБ | 1 420 мс | Полная блокировка сетевого пула домена на ~1.4 с: запросы выполняются 5 волнами по 6 сокетов, задержка отправки `POST /api/playback` до +920 мс |

### 2.3. Выводы по поведению клиента и бэкенда
1. **Клиентский bottleneck:** Браузер Google Chrome жестко лимитирует число одновременных TCP-соединений к одному HTTP/1.1 хосту шестью сокетами. При одновременном инстанциировании 30 объектов `new Audio()` браузер не может отправить 30 или 60 Range-запросов параллельно. Запросы с 7-го по 30-й принудительно ставятся в очередь сетевого планировщика браузера (`stalled`/`queued`), разделяясь на 5 последовательных волн по 6 запросов.
2. **Head-of-line blocking:** Захват всех 6 сокетов серией мелких Range-проб полностью парализует пропускную способность домена: любые критические запросы к API приложения (сохранение прогресса через `POST /api/playback`, фоновый опрос статуса, загрузка обложек) блокируются в очереди до высвобождения сокетов.
3. **Нагрузка на бэкенд и апстрим:** На стороне сервера `mpod` в каждый момент времени активно не более 6 параллельных горутин чтения медиапотока (по числу сокетов браузера), однако непрерывная смена сокетов и частые обрывы недочитанных потоков создают паразитный трафик к upstream CDN и избыточную нагрузку на SOCKS5-прокси.
