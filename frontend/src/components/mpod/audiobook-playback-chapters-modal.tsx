...
export function AudiobookPlaybackChaptersModal({
  ...
  return (
    <ModalScreen title={audiobook.title} onClose={onClose}>
      <Card className="flex w-full min-w-[320px] max-w-[720px] flex-col gap-5 overflow-hidden rounded-[20px] border-border bg-card p-4 shadow-xl sm:p-8">
        <div className="flex items-center gap-6">
          ...
          <Button
            ...
            onClick={onClose}
          >
            <HugeiconsIcon icon={MultiplicationSignIcon} className="size-6" />
          </Button>
        </div>

        <div className="mpod-scroll flex h-[408px] flex-col gap-1 overflow-y-auto pr-2">
          {tracks.map((track) => {
            ...
            const totalLabel = formatDuration(
              current ? currentDurationSeconds || track.duration : track.duration
            );
            ...
            return (
              ...
              >
                ...
                {!completed && durationLabel ? (
                  <span className="w-28 shrink-0 whitespace-nowrap text-right text-sm leading-5">
                    {durationLabel}
                  </span>
                ) : null}
                <Button
                  ...
                  variant="outline"
                  size="icon"
                  className="size-11 shrink-0 rounded-lg text-primary"
